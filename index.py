...
ratio = lambda x: Decimal(x[0] ** 2 + x[1] ** 2).sqrt()
getunitv = lambda x: vdiv(vdec(x), ratio(x))
moveto = lambda a, b, s: vadd(a, vmul(getunitv(vsub(b, a)), s))
getangle = lambda a, b: math.atan2((b[1] - a[1]), (b[0] - a[0]))
getaglv = lambda x: vdec((math.cos(x), math.sin(x)))
moveangle = lambda x, a, s: vadd(x, vmul(getaglv(a), s))
_moveto = lambda a, b, s: moveangle(a, getangle(a, b), s)
...
decodemsg = lambda x: attrdict(
    [(i.split("=")[0], eval(i.split("=")[1])) for i in x.decode("utf-8").split("&")]
)
decodemsgs = lambda x: [
    decodemsg(i.encode("utf-8")) for i in x.decode("utf-8")[:-1].split("$")
]
encodemsg = lambda x: ("&".join([f"{k}={v!r}" for k, v in x.items()]) + "$").encode(
    "utf-8"
)
...
class GameMap(GObject):
    ...
    def paint(self):
        ...
        pygame.draw.rect(
            self.screen, COLORS["black"], (*vsub(RSIZE, SSIZE), *vadd(SSIZE, (1, 1))), 1
        )
